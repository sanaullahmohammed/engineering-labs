# Design and Analysis for Algorithms Laboratory

## Programs List

### 1a

Create a Java class called Student with the following details as variables within it. <br>
(i) USN <br>
(ii) Name <br>
(iii) Programme <br>
(iv) Phone <br>
Write a Java program to create nStudent objects and print the USN, Name, Programme, and Phoneof these objects with suitable heading
...
(a) Dynamic Programming method  <br>
(b) Greedy method. <br>
...
Design and implement in Java to find a `subset` of a given set `S = {Sl, S2,.....,Sn}` of n positive integers whose SUM is equal to a given positive integer `d`. For example, if `S = {1, 2, 5, 6, 8}` and `d = 9`, there are two solutions `{1,2,6}` and `{1,8}`. Display a suitable message, if
